...
namespace
{
    ...
    {
        if (config.fft_num < 0 || !is_power_of_two(config.fft_num))
        {
            return false;
        }
...
namespace simplevox
{
    ...
    {
        feature_ = (int16_t*)malloc(sizeof(*feature_) * frame_num_ * coef_num_);
    }
...
#ifdef PC_DEBUG
        cfg = kiss_fftr_alloc(config.fft_num, 0, NULL, NULL);
#else
...
#ifdef PC_DEBUG
        kiss_fft_cpx out[512 / 2 + 1]; // 出力は (nfft/2 + 1) 個の複素数
        kiss_fftr(cfg, fft_data_.get(), out);
#else
        float* out = fft_data_.get();
        arm_rfft_fast_f32(&S, fft_data_.get(), out, 0);
#endif
